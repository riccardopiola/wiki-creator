...
class Parser {
 ...
 // The Object creator in use
  ...
  newComponent() {
    ...
    this.ObjectCreator = this.ObjectCreatorsArray[this.ObjectCreatorsArray.length - 1];
    this.componentTitle = 'standard';
  }
  ...
  checkWiki2(line) { // -> /** @wiki
    ...
    } else if (/class\s+(\w+)\s+/.test(line) && this.componentTitle === 'reading') { // Standard component name
      const result = /class\s+(\w+)\s+/.exec(line);
      this.ObjectCreator.addComponentTitle(result[1]);
      this.componentTitle = 'standard';
    } else if (/const\s+(\w+)\s+/.test(line) && this.componentTitle === 'reading') { // Standard component name
